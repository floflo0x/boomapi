...
curl_setopt_array($curl, array(
  ...
  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
  CURLOPT_CUSTOMREQUEST => 'POST',
  CURLOPT_POSTFIELDS => array('q2' => "select p.*,t.category_id,v.views from posts p join tags t on p.id = t.post_id join views v on p.id=v.post_id where t.category_id=$cId $filter limit 10 offset $offset"),
));
